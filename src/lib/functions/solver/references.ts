...
const generateStaticReference = async (reference: StaticReference) => {
	...
	if (fse.existsSync(file)) {
		console.info(`Reference for ${name} already exists.`)
		return
	}

	/// * Example: Turn UniswapV3Factory into UNISWAP_V3_FACTORY
	const bigName = name
		.split("")
		.map((letter, index) =>
			index === 0
	...
	let contractInterface = dedent`
        // Autogenerated file for ${name} through "solver references". Do not edit directly.
	...
	console.info(`Generated ${referencePath}/index.ts`)
...
const generateDynamicReference = (reference: DynamicReference) => {
	...
	try {
		...
		Object.entries(contractSources).forEach(([sourceKey, value]) => {
			...
			console.info(`Generated ${directory}/${filename}`)
		})
	} catch (error: any) {
		console.error(
...
export const generateReferences = async (network: Network) => {
	...
	console.log(`References generated for ${references.length} contracts.`)
}
